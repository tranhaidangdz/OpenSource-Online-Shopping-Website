...
//search product: 	tìm theo từ khóa người dùng nhập 
...
if (isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])) {
	if (isset($_POST["get_seleted_Category"])) {
		...
		$keyword = mysqli_real_escape_string($con, $_POST["keyword"]);  //Thêm mysqli_real_escape_string để tránh lỗi SQL injection.
		$sql = "SELECT * FROM products,categories 
		        WHERE product_cat=cat_id 
		        AND (product_title LIKE '%$keyword%' 
		        OR product_desc LIKE '%$keyword%' 
		        OR product_keywords LIKE '%$keyword%')";
	}
	...
	if (mysqli_num_rows($run_query) > 0) {
		...
		echo "<div class='alert alert-danger'>Không tìm thấy sản phẩm nào phù hợp!</div>";
	}
}
